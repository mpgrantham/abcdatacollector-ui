...
const Datasheet = ({ observed }: DatasheetProps) => {

    const buildAbcSection = (label: string, values: Abc[], displayRightBorder: boolean) => {
        const abcs = values 
          ?   values.map((v) => {
                return <View style={styles.abcEntry} key={v.id.toString()}><Text>{v.value}</Text></View>
              })
          : []
    ...
    return (
        <Document>
            <Page size="A4" style={styles.page} orientation="landscape" wrap>
                <View style={styles.row}><Text style={styles.title}>Observed: {observed.observedName}</Text></View>
                ...
                <View style={styles.row} break><Text style={styles.title}>Observed: {observed.observedName}</Text></View>
